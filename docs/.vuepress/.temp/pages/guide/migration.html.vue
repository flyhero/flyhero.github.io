<template><div><h1 id="migrating-from-v1" tabindex="-1"><a class="header-anchor" href="#migrating-from-v1" aria-hidden="true">#</a> Migrating from v1</h1>
...
<li>VuePress v2 is developed with TypeScript, so it provides better TS support now. It's highly recommended to use TypeScript to develop plugins and themes. VuePress config file also supports TypeScript, and you can use <code v-pre>.vuepress/config.ts</code> directly.</li>
<li>VuePress v2 supports both Webpack and Vite as bundler. Now Vite is the default bundler, while you can still choose to use Webpack. You can even use Vite in dev mode to get better development experience, and use Webpack in build mode to get better browser compatibility.</li>
<li>VuePress v2 is now released as pure ESM packages, and CommonJS config files are no longer supported.</li>
</ul>
...
<p>Config file should be in ESM format, and CommonJS format config file is no longer supported.</p>
<div class="language-diff line-numbers-mode" data-ext="diff"><pre v-pre class="language-diff"><code>// .vuepress/config.js
...
</span></span></code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="theme" tabindex="-1"><a class="header-anchor" href="#theme" aria-hidden="true">#</a> theme</h4>
<p>Using a theme via string is not supported. Import the theme directly.</p>
<div class="language-diff line-numbers-mode" data-ext="diff"><pre v-pre class="language-diff"><code><span class="token deleted-sign deleted"><span class="token prefix deleted">-</span><span class="token line"> module.exports = {
...
<p>Using a plugin via string is not supported. Import the plugin directly.</p>
<div class="language-diff line-numbers-mode" data-ext="diff"><pre v-pre class="language-diff"><code><span class="token deleted-sign deleted"><span class="token prefix deleted">-</span><span class="token line"> module.exports = {
...
<p>Default value is changed from <code v-pre>() =&gt; true</code> to <code v-pre>true</code>.</p>
...
<p>Default value is changed from <code v-pre>false</code> to <code v-pre>true</code>.</p>
...
<p>Moved to <RouterLink to="/reference/config.html#markdown-headers">markdown.headers</RouterLink>.</p>
...
<p>All webpack related configs are moved to options of <code v-pre>@vuepress/bundler-webpack</code>, including:</p>
<ul>
...
<li><code v-pre>evergreen</code>: default value is changed from <code v-pre>false</code> to <code v-pre>true</code></li>
</ul>
<div class="language-diff line-numbers-mode" data-ext="diff"><pre v-pre class="language-diff"><code><span class="token deleted-sign deleted"><span class="token prefix deleted">-</span><span class="token line"> module.exports = {
...
</span></span></code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Please refer to <RouterLink to="/guide/bundler.html">Guide &gt; Bundler</RouterLink>.</p>
...
<div class="language-yaml line-numbers-mode" data-ext="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">head</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Has the same structure with:</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="permalink-patterns-change" tabindex="-1"><a class="header-anchor" href="#permalink-patterns-change" aria-hidden="true">#</a> Permalink Patterns Change</h3>
<ul>
...
<p>The stylus palette system of VuePress v1 (i.e. <code v-pre>styles/palette.styl</code> and <code v-pre>styles/index.styl</code>) is no longer provided by VuePress Core.</p>
...
<p>Renamed to <code v-pre>.vuepress/client.{js,ts}</code>, and the usage has been changed, too.</p>
<p>See <RouterLink to="/advanced/cookbook/usage-of-client-config.html">Advanced &gt; Cookbook &gt; Usage of Client Config</RouterLink>.</p>
...
<p>You need to use <RouterLink to="/reference/plugin/register-components.html">@vuepress/plugin-register-components</RouterLink>, or register your components manually in <code v-pre>.vuepress/client.{js,ts}</code>.</p>
...
<p>You need to import and set your local theme via <RouterLink to="/reference/config.html#theme">theme</RouterLink> option.</p>
...
<ul>
<li><code v-pre>-c, --cache [cache]</code>: changed to <code v-pre>--cache &lt;cache&gt;</code>, which means that the shorthand <code v-pre>-c</code> is not for <code v-pre>cache</code> option, and the value of <code v-pre>cache</code> option is not optional.</li>
<li><code v-pre>--no-cache</code>: renamed to <code v-pre>--clean-cache</code> .</li>
</ul>
...
<ul>
<li><code v-pre>&lt;CodeGroup /&gt;</code> and <code v-pre>&lt;CodeBlock /&gt;</code> renamed to <code v-pre>&lt;CodeGroup /&gt;</code> and <code v-pre>&lt;CodeGroupItem /&gt;</code></li>
<li><code v-pre>&lt;Badge /&gt;</code>
<ul>
<li><code v-pre>$badgeErrorColor</code> palette variable renamed to <code v-pre>$badgeDangerColor</code></li>
<li><code v-pre>type</code> prop only accepts <code v-pre>tip</code>, <code v-pre>warning</code> and <code v-pre>danger</code> now</li>
</ul>
</li>
</ul>
...
<p>Default theme config has been changed a lot. You'd better check the config reference of v2 default theme to migrate it properly.</p>
...
<ul>
<li>You cannot use other plugins in your plugin anymore, which avoids lots of potential issues caused by plugin nesting. If your plugin depends on other plugins, you could list them in the docs to ask users import them manually. Alternatively, you can provide users with an array of plugins for convenience.</li>
<li>Most of the v1 hooks have equivalents in v2. The only exception is <code v-pre>extendsCli</code>, which has been removed.</li>
<li>Webpack related hooks are removed, because VuePress Core has decoupled with webpack. You can try to use <RouterLink to="/reference/plugin-api.html#extendsbundleroptions">extendsBundlerOptions</RouterLink> hook for similar purpose, and make sure to work with all bundlers.</li>
</ul>
<p>For more detailed guide about how to write a plugin in v2, see <RouterLink to="/advanced/plugin.html">Advanced &gt; Writing a Plugin</RouterLink>.</p>
...
<li><code v-pre>ready</code>: renamed to <code v-pre>onPrepared</code></li>
<li><code v-pre>updated</code>: renamed to <code v-pre>onWatched</code></li>
<li><code v-pre>generated</code>: renamed to <code v-pre>onGenerated</code></li>
<li><code v-pre>additionalPages</code>: removed, use <code v-pre>app.pages.push(createPage())</code> in <code v-pre>onInitialized</code> hook</li>
<li><code v-pre>clientDynamicModules</code>: removed, use <code v-pre>app.writeTemp()</code> in <code v-pre>onPrepared</code> hook</li>
<li><code v-pre>enhanceAppFiles</code>: removed, use <code v-pre>clientConfigFile</code> hook</li>
<li><code v-pre>globalUIComponents</code>: removed, use <code v-pre>clientConfigFile</code> hook</li>
<li><code v-pre>clientRootMixin</code>: removed, use <code v-pre>clientConfigFile</code> hook</li>
<li><code v-pre>extendMarkdown</code>: renamed to <code v-pre>extendsMarkdown</code></li>
<li><code v-pre>chainMarkdown</code>: removed</li>
<li><code v-pre>extendPageData</code>: renamed to <code v-pre>extendsPage</code></li>
...
<p>Although we do not allow using other plugins in a plugin anymore, you can still use plugins in your theme.</p>
<p>Some major breaking changes:</p>
<ul>
<li>There is no <strong>conventional theme directory structure</strong> anymore.
<ul>
<li>The file <code v-pre>theme/enhanceApp.js</code> will not be used as client app enhance file implicitly. You need to specify it explicitly in <code v-pre>clientConfigFile</code> hook.</li>
<li>Files in <code v-pre>theme/global-components/</code> directory will not be registered as Vue components automatically. You need to use <RouterLink to="/reference/plugin/register-components.html">@vuepress/plugin-register-components</RouterLink>, or register components manually in <code v-pre>clientConfigFile</code>.</li>
<li>Files in <code v-pre>theme/layouts/</code> directory will not be registered as layout components automatically. You need to specify it explicitly in <code v-pre>layouts</code> option in <code v-pre>clientConfigFile</code>.</li>
<li>Files in <code v-pre>theme/templates/</code> directory will not be used as dev / ssr template automatically. You need to specify theme explicitly in <code v-pre>templateBuild</code> and <code v-pre>templateDev</code> option.</li>
<li>Always provide a valid js entry file, and do not use <code v-pre>&quot;main&quot;: &quot;layouts/Layout.vue&quot;</code> as the theme entry anymore.</li>
</ul>
</li>
<li><code v-pre>themeConfig</code> is removed from user config and site data. To access the <code v-pre>themeConfig</code> as you would via <code v-pre>this.$site.themeConfig</code> in v1, we now recommend using the <RouterLink to="/reference/plugin/theme-data.html">@vuepress/plugin-theme-data</RouterLink> plugin and its <code v-pre>useThemeData</code> composition API.</li>
...
<li>For scalability concerns, <code v-pre>this.$site.pages</code> is not available any more.</li>
</ul>
<p>For more detailed guide about how to write a theme in v2, see <RouterLink to="/advanced/theme.html">Advanced &gt; Writing a Theme</RouterLink>.</p>
...
<p>Removed.</p>
<p>Now you need to specify layout component in the client config file of your theme.</p>
<p>See <RouterLink to="/advanced/theme.html">Advanced &gt; Writing a theme</RouterLink>.</p>
...
<p>You can still inherit a parent theme with <code v-pre>extends: parentTheme()</code>, which will extends the plugins, layouts, etc.</p>
<p>You can refer to <RouterLink to="/reference/default-theme/extending.html">Default Theme &gt; Extending</RouterLink> for how to extend default theme.</p>
<p>The <code v-pre>@theme</code> and <code v-pre>@parent-theme</code> aliases are removed by default, but you can still make a extendable theme with similar approach, see <RouterLink to="/advanced/cookbook/making-a-theme-extendable.html">Advanced &gt; Cookbook &gt; Making a Theme Extendable</RouterLink>.</p>
</div></template>
