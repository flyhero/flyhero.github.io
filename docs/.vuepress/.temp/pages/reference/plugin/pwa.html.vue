...
<NpmBadge package="@vuepress/plugin-pwa" /><p>Make your VuePress site a <a href="https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps" target="_blank" rel="noopener noreferrer">Progressive Web Application (PWA)<ExternalLinkIcon/></a>.</p>
<p>This plugin uses <a href="https://developers.google.com/web/tools/workbox/modules/workbox-build" target="_blank" rel="noopener noreferrer">workbox-build<ExternalLinkIcon/></a> to generate service worker file, and uses <a href="https://github.com/yyx990803/register-service-worker" target="_blank" rel="noopener noreferrer">register-service-worker<ExternalLinkIcon/></a> to register service worker.</p>
<h2 id="usage" tabindex="-1"><a class="header-anchor" href="#usage" aria-hidden="true">#</a> Usage</h2>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">npm</span> i <span class="token parameter variable">-D</span> @vuepress/plugin-pwa@next
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> pwaPlugin <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@vuepress/plugin-pwa'</span>
...
<p>To make your website fully compliant with PWA, you need to create a <a href="https://developer.mozilla.org/en-US/docs/Web/Manifest" target="_blank" rel="noopener noreferrer">Web app manifests<ExternalLinkIcon/></a> file and set the icons, colors, etc. for your PWA.</p>
<p>You need to put your manifest file and icons into the <RouterLink to="/guide/assets.html#public-files">public files directory</RouterLink>. In the following example, we assume that you are using the default public directory <code v-pre>.vuepress/public</code>.</p>
<ol>
...
<p>Make sure the path of icons matches the <code v-pre>icons</code> field in your manifest file:</p>
<ul>
...
<p>Some tools can help to do that. For example, <a href="https://realfavicongenerator.net/" target="_blank" rel="noopener noreferrer">Favicon Generator<ExternalLinkIcon/></a> would help you to generate icons together with a sample manifest file.</p>
</div>
<ol start="3">
<li>Set tags in head</li>
</ol>
<p>You also need to set some tags via <RouterLink to="/reference/config.html#head">head</RouterLink> option to <a href="https://developer.mozilla.org/en-US/docs/Web/Manifest#deploying_a_manifest_with_the_link_tag" target="_blank" rel="noopener noreferrer">deploy the manifest<ExternalLinkIcon/></a>:</p>
...
<p>This plugin accepts all parameters of workbox-build's <a href="https://developers.google.com/web/tools/workbox/reference-docs/latest/module-workbox-build#.generateSW" target="_blank" rel="noopener noreferrer">generateSW method<ExternalLinkIcon/></a> in its options, except <code v-pre>globDirectory</code> and <code v-pre>swDest</code>.</p>
<p>For example, you can set <code v-pre>skipWaiting: true</code> to auto activate the new service worker once it is ready:</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span>
  plugins<span class="token operator">:</span> <span class="token punctuation">[</span>
    <span class="token function">pwaPlugin</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">]</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>But if you omit <code v-pre>skipWaiting</code> or set it to <code v-pre>false</code>, you have to activate the new service worker manually:</p>
<ul>
...
<p>The service worker file will only be generated in <code v-pre>build</code> mode.</p>
</li>
</ul>
...
<p>You can add listener function to events that provided by <a href="https://github.com/yyx990803/register-service-worker" target="_blank" rel="noopener noreferrer">register-service-worker<ExternalLinkIcon/></a>.</p>
</li>
<li>
<p>Example:</p>
</li>
</ul>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> usePwaEvent <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'@vuepress/plugin-pwa/client'</span>
...
<li>
<p>Details:</p>
<p>Call <a href="https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerGlobalScope/skipWaiting" target="_blank" rel="noopener noreferrer">skipWaiting()<ExternalLinkIcon/></a> to activate the waiting service worker.</p>
</li>
<li>
<p>Example:</p>
</li>
